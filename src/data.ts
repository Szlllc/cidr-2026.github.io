  ...
  "qna": [
    {
      ...
      "a": "数据库领域重新关注Yannakakis 算法，是因为现代研究者试图通过位图过滤器（Bitmap Filters）这一务实的工程手段，在高性能数据库实践中低成本地实现该算法所描述的实例最优。传统的二元连接在处理复杂多表关联时常产生巨大的中间结果，而 Yannakakis 算法通过一系列半连接缩减，能保证中间过程不产生无用的数据 ；工业界发现，利用位图过滤器的轻量级特性配合代价优化器，可以在不引入昂贵标准半连接算子的情况下，使执行引擎达到与该经典算法一致的理论缩减效率，从而在处理无环和复杂有环查询时获得极大的性能增益。",
    ...
    },
    {
      ...
      "a": "因为SQL Server 的做法没有引入全新的、不兼容的执行引擎，而是利用现有的拉取式框架、共享存储和 Cascades 优化器，通过一系列工程上的权衡和优化，达到了媲美顶尖学术算法的效果，同时保持了系统的稳定性和通用性 。工程上的优化如：SQL Server 不仅能在原始表扫描阶段进行过滤，还可以在连接产生的中间临时结果上构建和应用位图，进一步压减后续复杂连接的处理量 。这种灵活性使得它能够应对极其复杂的查询拓扑结构，作者提到这在很多纯理论研究中并未被充分讨论。",
    ...
    },
    {
      ...
      "a": "因为位图非常紧凑，传递开销极小。传统的半连接需要传输实际的数据键值，可能会导致巨大的网络或内存负载；而位图只需传输少量字节，却能达到同样的过滤效果，且能轻松集成到现有的拉取式引擎中，不需要重构执行框架。"
    },
    {
      "q": "为什么说 SQL Server 的位图过滤不仅仅是简单的“过滤”，而是“继承了 Yannakakis 的精神”？",
      "a": "因为 SQL Server 支持级联过滤（Cascading Filters） 。这类似于 Yannakakis 算法的缩减过程。当一个 Hash Join 产生的过滤位图被下推到更深层的扫描算子，甚至被传递给另一个 Join 算子时，它在逻辑上完成了一次“远距离半连接” 。这意味着一个表产生的过滤条件可以层层渗透，让整个执行树中的所有相关表都提前减少数据量。",
      "image": "figures/2-4.png"
    },
    {
      "q": "SQL Server 中的位图过滤器在面对复杂的“环形查询”时表现如何？",
      "a": "虽然 Yannakakis 算法理论上针对无环查询，但 SQL Server 的位图过滤机制可以处理任意连接图。由于位图可以从多个 Build 侧生成并同时推送到同一个 Probe 侧，它实际上为有环查询提供了一种接近理论上限的加速效果。",
      "image": "figures/2-5.png"
    },
    {
      "q": "SQL Server 的位图过滤目前还有什么局限性吗？",
      "a": "虽然它很强大，但仍受限于优化器的成本估计。如果由于基数估计不准导致优化器认为位图开销大于收益，它就不会采用该计划。此外，在极度稀疏或键值范围巨大的情况下，位图的过滤精度和内存消耗之间也存在权衡。"
    },
    {
    ...
    },
    {
      ...
      "a": "核心难点不在于“位图”本身，而在于优化器的集成。你需要让优化器能够感知到位图过滤器的存在，将其建模为一个代价可计量的物理操作，并赋予位图过滤器跨算子移动的能力 。只有优化器学会了如何正确放置位图，才能真正发挥出类似 Yannakakis 算法的威力。"
    }
  ],
  "extendedResources": [
    {
