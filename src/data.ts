...
export const expressPapers = [
  ...
  },
  {
    ...
    "abstract": "该研究聚焦于LSM 存储架构在关系型 OLTP 场景下跨多个层级读取数据的性能成本方面的挑战。该研究介绍了 Salesforce 如何**通过修改 PostgreSQL 内核并引入 LSM 存储引擎来支撑超大规模多租户 OLTP 业务**，主要提出了针对 LSM 读性能的**三项额外优化**：**缓存技术**以实现类似 RowID 的直接寻址，利用**基于前缀树的 CRaFT 过滤器**跳过无效的范围扫描，并设计了**早期墓碑裁剪(Early Tombstone Pruning)策略**以优化队列型业务的处理。",
  ...
  },
  {
    ...
    "abstract": "该研究深入剖析了 SQL Server 如何将位图过滤器无缝集成到拉取式执行模型中，从而隐式实现了实例最优的连接计划。该研究证明，通过 Cascades 优化器将位图开销建模为原生代价维度，并允许在中间结果上动态构建和下推过滤器，可以显著减少非必要的数据扫描和连接探测开销，展示了工业界如何通过务实的工程设计将复杂的理论算法转化为普适的加速能力。",
...
export const deepDivePapers = [
  {
    ...
    "prerequisites": [
      ...
      "集成式 vs. 专用向量数据库：集成式数据库（如 pgvector）在传统关系型数据库（如 PostgreSQL）内部构建向量功能，虽能利用SQL和事务，但受限于传统存储架构，性能通常较弱；专用向量数据库（如 Milvus, Pinecone）为向量检索量身定制，性能极高但破坏了数据一致性。",
    ...
    "qna": [
      {
        "q": "现有的基于PostgreSQL 的集成式数据库的弊端是什么？",
      ...
      {
        "q": "为了使集成式数据库达到专用数据库级别性能，本研究做了哪些工作？",
        "a": "本研究提出**PostgreSQL-V**，一个在 PostgreSQL 中实现**快速向量搜索**的新系统。与之前通过重用 PostgreSQL 的面向页面结构来继承旧有开销的工作（如 pgvector）不同，PostgreSQL-V 采用了一种新颖的架构设计，将**向量索引**从 PostgreSQL 的核心引擎中**解耦**。不再把向量索引塞进 PostgreSQL 的数据页里，而是将其移到引擎外部，作为一个**独立**的存储组件。内部通过 LSM 架构（类似 RocksDB）管理向量，外部向量索引**不再受限**于PostgreSQL的页面格式和缓冲区管理，可以**直接**调用高效的原生向量库（如Faiss或hnswlib），利用内存指针直接访问数据，从而消除架构瓶颈。",
        "image": "figures/1-1.png"
      },
      {
        "q": "既然索引跑到了数据库外面，那如何保证数据的一致性？",
        "a": "PostgreSQL-V通过引入“状态页（Status Pages）”机制和将向量操作挂载到系统的预写日志（WAL）中来保证数据的一致性。\n\n* **“状态页（Status Pages）”机制**，在 PostgreSQL 内部**记录**每个向量的**当前状态**（如是否已删除或更新），检索时会将外部索引返回的结果与内部状态页进行比对过滤，确保用户查到的永远是最新的正确数据；\n* **将向量操作挂载到WAL**：实际的索引操作在解耦组件中进行。即使解耦组件崩溃，也能通过受**WAL保护**的状态页作为“单一真理来源”来**修复**索引，不会出现“主表删了数据，索引没删”这种情况。在系统重启恢复时，它会扫描日志并重放未同步到外部索引的操作，保证了“**解耦但不失稳**”。",
        "image": "figures/1-2.png"
      },
      {
        "q": "PostgreSQL-V 支持哪些类型的向量索引？",
        "a": "得益于其解耦设计，**PostgreSQL-V** 可以直接“套用”外部成熟的库，从而轻松支持**几乎所有**主流算法，包括基于图的 HNSW、基于聚类的 IVF 以及乘积量化（PQ）等。"
      },
      {
        "q": "在实际测试中，PostgreSQL-V的性能表现如何？",
        "a": "在向量搜索方面，**PostgreSQL-V**比pgvector快**6.5**到**8.9**倍，比PASE快**6**倍。同时，**PostgreSQL-V**的索引构建速度也大幅提升，且占用的存储空间更小。最重要的是，达到了**与专用向量数据库**（如Faiss）**持平**的性能水平。",
      ...
      },
      {
        ...
        "a": "将向量索引与数据库核心引擎**解耦**，是大幅提升集成式向量数据库性能的关键。在实现向量索引解耦后，为保证索引一致性，需将向量索引的**元数据**与索引**内容分离**，依托宿主数据库的事务机制管理元数据，并基于元数据为索引内容设计定制化的**一致性保障机制**。"
      }
    ],
    "extendedResources": [
      {
  ...
  "qna": [
    {
      ...
      "a": "数据库领域重新关注 **Yannakakis 算法**，是因为现代研究者试图通过**位图过滤器（Bitmap Filters）**这一务实的工程手段，在高性能数据库实践中**低成本**地实现该算法所描述的**实例最优**。**传统**的二元连接在处理复杂多表关联时常产生**巨大的中间结果**，而 **Yannakakis 算法**通过一系列半连接缩减，能保证中间过程**不产生无用**的数据；工业界发现，利用位图过滤器的轻量级特性配合代价优化器，可以在**不引入昂贵**标准半连接算子的情况下，使执行引擎达到与该经典算法**一致**的**理论缩减效率**，从而在处理无环和复杂有环查询时获得极大的性能增益。",
      "image": "figures/2-1.png"
    },
    {
      "q": "既然这么强，为什么数据库不默认全盘采用 Yannakakis？",
      "a": "因为Yannakakis 需要**额外**的**半连接开销**。如果你的数据本身选择性很高，或者连接后的结果集依然很大，那么提前做两轮半连接反而是浪费 CPU。**SQL Server 的做法**是将其纳入 **Cascades 优化器**，由**代价模型**决定是否开启这种“Yannakakis 模式”。Cascades 优化器将**位图**的**创建**和**应用**视为逻辑算子的不同物理实现。在处理查询时，Cascades 优化器会**计算**位图产生的**开销**（如内存和 CPU）、后续算子开销和过滤带来的**收益**（如节省的 IO）并根据估算的代价来**决定**是否放置、在哪里放置以及下推多深。"
    },
    {
      "q": "SQL Server 的做法比纯粹的 Yannakakis 算法更优吗？",
      "a": "在生产环境下是的。**纯算法**需要严格的树分解和复杂的两阶段遍历，工程**实现极难**且**泛化性差**。**SQL Server** 将其转化为一种“基于代价的插件”：**Cascades优化器**会**计算**位图的**过滤收益**，如果划算就插入，不划算就不用。这种**务实**的策略让它在保持系统通用性的同时，获得了**接近理论最优**的加速。",
      "image": "figures/2-2.png"
    },
    {
      "q": "为什么说 SQL Server 的做法是“务实”的？",
      "a": "因为SQL Server 的做法**没有引入全新的**、**不兼容的**执行引擎，而是**利用现有**的**拉取式框架**、**共享存储**和 **Cascades 优化器**，通过一系列**工程**上的**权衡和优化**，达到了**媲美**顶尖学术算法的效果，同时保持了系统的稳定性和通用性。工程上的优化如：SQL Server 不仅能在**原始表**扫描阶段进行**过滤**，还可以在连接产生的**中间临时结果**上**构建和应用位图**，进一步压减后续复杂连接的处理量。这种灵活性使得它能够应对极其复杂的查询拓扑结构，作者提到这在很多纯理论研究中并未被充分讨论。",
      "image": "figures/2-3.png"
    },
    {
      "q": "SQL Server 中的位图过滤器（Bitmap Filter）是如何工作的？",
      "a": "在执行 Join 操作时，MSSQL 会在 Hash Join 的 **Build 侧**（构建哈希表的一侧）**创建**一个位图，记录所有出现的键值；然后将这个位图**下推**到 **Probe 侧**（探测侧）的扫描算子中。这样，Probe 侧在读取数据时，就能立即**跳过**那些位图中不存在的行，极大地减少了后续进入 Join 算子的数据量。"
    },
    {
      "q": "为什么位图过滤比传统的“半连接”算法更受工业界青睐？",
      "a": "因为**位图**非常紧凑，传递**开销**极**小**。**传统**的半连接需要传输**实际**的数据键值，可能会导致巨大的网络或内存负载；而**位图**只需传输**少量**字节，却能达到同样的过滤效果，且能轻松集成到现有的拉取式引擎中，不需要重构执行框架。"
    },
    {
      "q": "为什么说 SQL Server 的位图过滤不仅仅是简单的“过滤”，而是“继承了 Yannakakis 的精神”？",
      "a": "因为 SQL Server 支持**级联过滤（Cascading Filters）**。这**类似**于 **Yannakakis 算法**的**缩减**过程。当一个 Hash Join 产生的过滤位图**被下推**到更深层的扫描算子，甚至**被传递**给另一个 Join 算子时，它在逻辑上完成了一次“**远距离半连接**”。这意味着一个表产生的**过滤**条件可以**层层渗透**，让整个执行树中的所有相关表都**提前减少**数据量。",
      "image": "figures/2-4.png"
    },
    {
      "q": "SQL Server 中的位图过滤器在面对复杂的“环形查询”时表现如何？",
      "a": "虽然 **Yannakakis 算法**理论上针对**无环查询**，但 **SQL Server** 的**位图过滤**机制可以处理**任意连接图**。由于位图可以从**多个** **Build 侧**生成并同时**推送到同一个** Probe 侧，它实际上为有环查询提供了一种**接近理论上限**的加速效果。",
      "image": "figures/2-5.png"
    },
    {
      "q": "SQL Server 的位图过滤目前还有什么局限性吗？",
      "a": "虽然它很强大，但仍**受限**于优化器的**成本估计**。如果由于基数估计不准导致优化器认为位图开销大于收益，它就不会采用该计划。此外，在**极度稀疏**或**键值范围巨大**的情况下，位图的过滤精度和内存消耗之间也存在权衡。"
    },
    {
    ...
    },
    {
      ...
      "a": "核心难点不在于“位图”本身，而在于**优化器的集成**。你需要让优化器能够**感知**到**位图过滤器**的存在，将其**建模**为一个代价可计量的物理操作，并赋予位图过滤器**跨算子移动**的能力。只有**优化器**学会了如何**正确放置位图**，才能真正发挥出类似 Yannakakis 算法的威力。"
    }
  ],
  "extendedResources": [
    {
  ...
  },
  {
    ...
    "prerequisites": [ // 前导知识，以【关键词：解读】的形式呈现
      ...
      '多世界隔离(Multi-world Isolation)： 具体内容及作用：针对智能体在进行数据更新时喜欢并行测试大量“假设（What-if）”场景的特点，提出的一种新型事务并发控制机制。在此机制下，每个分支在逻辑上完全隔离，但在物理存储上高度重叠。了解这一点对于思考传统MVCC（多版本并发控制）如何向极大规模并行分支和快速回滚演进非常有帮助。 '
    ], 
    "qna": [ // QA 对，如若涉及图片，给出图片的链接（本地），后续将图片一起发送给我
      {
